...
from Config import print_step_results, p_pool_size, n_PTRs, test_epoch
...
for train_scope in np.linspace(5, 50, 5):
...
for i, each in enumerate(F1s):
    plt.plot(np.linspace(0, 0.5, 101), each, label=f"train scope = {int(np.linspace(5, 50, 5)[i])}")
plt.legend()
plt.show()
