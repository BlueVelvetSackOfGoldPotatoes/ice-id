...
from Config import train_scope, print_step_results, p_pool_size, n_PTRs, test_epoch
...
for train_proportion in 1 / np.linspace(1000, 10000, 5):
    ...
    for i, r1 in tqdm(enumerate(df)):
        if random.random() < train_proportion:
            for j in range(i + 1, i + 1 + train_scope):
                if j >= len(df):
                    continue
                r2 = df[j]
...
for i, each in enumerate(F1s):
    plt.plot(np.linspace(0, 0.5, 101), each, label=f"train proportion = 1 / {int(np.linspace(1000, 10000, 5)[i])}")
plt.legend()
plt.show()
